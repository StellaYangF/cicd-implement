...
Docker镜像的概念有点像 “代码模版”，镜像内是一个操作系统。

镜像一般都比较小，由多个层构成。可以看到，下图 Nginx 镜像最底层是 Ubuntu15.04 镜像，上层才是应用服务等其他层。在推送镜像/拉取镜像时，Docker不会拉取完整镜像，只会拉取有更改的那一层，这样可以做到快速更新镜像。例如下图是Nginx镜像的层级结构。
...
首先在防火墙添加 8080 和 50000 端口的放行，并重载防火墙
...
点击 Jenkins 首页 -> 左侧导航 -> 新建任务 -> 构建一个自由风格的软件项目
...
容器启动后，需要修改 GitLab 配置文件，修改 GitLab 主服务地址和 SSH 服务地址
